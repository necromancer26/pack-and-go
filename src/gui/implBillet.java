...
public class implBillet implements interBillet {
    ...
    public void insert(modelBillet m) {
        ...
        try {
            ps = k.connect().prepareStatement("insert into billet (nom, prenom,cin,date_de_naissance,classe,volid) values(?,?,?,?,?,?)");
           // ps.setInt(1, m.getId());
            ps.setString(1,m.get_nom());
            ps.setString(2,"prenom");
            ps.setInt(3,11111);
            ps.setDate(4,java.sql.Date.valueOf("1990-01-02"));
            ps.setString(5,m.get_classe());
            ps.setInt(6,m.getVolid());
