...
public class implVol implements interVol {
    ...
    public void update(modelVol m) {
        ...
        PreparedStatement ps;
        try {
            ps = k.connect().prepareStatement("UPDATE vols SET compagnie_aerien=?, depart=?, destination=?, date_depart=?, date_arrive=?, prix=?, type_avion=?, duree=?, image=? WHERE id = ?");
            //    ps.setInt(1,m.getId());
